...
function Buket2() {
  ...
  function a(matn) {
    // Matndagi katta harflar oldidan vergul qo'yish
    var yangiMatn = matn.replace(/([A-Z])/g, ", $1");

    // Agar matn boshida vergul bo'lmasa, boshiga vergul qo'shish
    if (yangiMatn.charAt(0) === ",") {
      yangiMatn = yangiMatn.slice(2);
    }
        ...
        <div className="w-[100%] px-4 lg:w-[94%] mx-auto">
          ...
          <div className="flex py-[40px] items-center flex-col md:flex-row">
            <div className="flex md:flex-row flex-col gap-x-3 gap-y-4 text-center w-[93%]  my-3 md:my-0 ml-0 md:ml-5">
              <p
                onClick={SortBySum1}
                className="bg-white mr-0 md:mr-5 rounded-2xl font-medium text-[14px] md:text-[16px] lg:text-[20px] py-2 cursor-pointer tracking-[1px] text-center text-[#343434] px-2 md:px-2 border-2 line-clamp-1"
              >
                до 5000 руб
              </p>
              <p
          ...
          <div className="rounded-3xl relative py-5 flex flex-wrap gap-x-4 gap-y-4 mb-[40px]">
            {flowers1.length > 0 ? (
